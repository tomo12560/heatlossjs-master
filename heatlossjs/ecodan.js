...
function get_ecodan_cop(outlet, outside, load) {
  var ecodan = {
    min: {
      data: [
        ...
        [2.1, 2.34, 2.35, 2.86, 3.41, 3.73, 3.91, 4.23], // 45
        [null, 2.1, 2.12, 2.54, 3.07, 3.32, 3.46, 3.71], // 50
    ...
    mid: {
      data: [
        //-15,  -10,   -7,    2,    7,   12,   15,   20
        [null, 3.64, 3.85, 4.9, 5.89, 6.58, 7.08, 7.98], // 25
        [2.66, 3.01, 3.25, 3.54, 4.63, 5.35, 5.79, 6.54], // 35
        [2.38, 2.68, 2.87, 3.35, 4.18, 4.66, 4.97, 5.48], // 40
        [2.1, 2.34, 2.5, 3.15, 3.73, 3.98, 4.15, 4.43], // 45
        [null, 2.1, 2.25, 2.78, 3.23, 3.43, 3.56, 3.78], // 50
        [null, 1.86, 2.0, 2.41, 2.74, 2.88, 2.98, 3.14], // 55
        [null, null, 1.7, 2.05, 2.56, 2.59, 2.62, 2.68], // 60 (note 1.7 at -7 is not from datasheet)
      ],
    },
    max: {
      data: [
        //-15,  -10,   -7,    2,    7,   12,   15,   20
        [null, 3.3, 3.6, 4.2, 5.48, 6.2, 6.65, 7.41], // 25
        [2.44, 2.78, 3.0, 3.5, 4.5, 4.98, 5.28, 5.79], // 35
        [2.22, 2.51, 2.7, 3.15, 4.01, 4.37, 4.59, 4.98], // 40
        [2.0, 2.25, 2.4, 2.8, 3.52, 3.75, 3.91, 4.16], // 45
        [null, 2.05, 2.16, 2.47, 3.1, 3.27, 3.38, 3.57], // 50
        [null, 1.85, 1.92, 2.13, 2.68, 2.78, 2.85, 2.97], // 55
        [null, null, null, 1.8, 2.26, 2.3, 2.33, 2.38], // 60
      ],
    },
  };
  ...
  var c1 =
    a +
    ((b - a) * (outside - cols[outside_index])) /
      (cols[outside_index + 1] - cols[outside_index]);
  ...
  var cop =
    c1 +
    ((c2 - c1) * (outlet - rows[outlet_index])) /
      (rows[outlet_index + 1] - rows[outlet_index]);
