...
var app = new Vue({
  ...
  methods: {
    ...
    add_element_type: function () {
      ...
      if (last != false) {
        config.element_type["Element " + index] = JSON.parse(
          JSON.stringify(last)
        );
      } else {
    ...
    change_room_name: function (e, old_room_name) {
      ...
      config.rooms[new_room_name] = JSON.parse(
        JSON.stringify(config.rooms[old_room_name])
      );
    ...
    file_save: function () {
      ...
      download_data(
        "heatlossjs_" + config.project_name + "_" + datestr + ".json",
        JSON.stringify(filtered_config, null, 2)
      );
    },
...
function calculate() {
  ...
  if (config.heating_MWT == null || config.heating_MWT == "")
    config.heating_MWT = 0;
  ...
  for (var z in config.rooms) {
    ...
    var infiltration_wk =
      room.air_change_an_hour * air_change_factor * room.volume;
    ...
    for (var i in room.ufh) {
      ...
      var m =
        86.7 * Math.pow(ufh.flooring, 2) - 31.543 * ufh.flooring + 5.74385;
      var c =
        424.7 * Math.pow(ufh.flooring, 2) - 156.403 * ufh.flooring + 28.70185;
  ...
  // Heat pump model
  var heatpump_deltaT =
    config.house.total_heat_output / ((config.heatpump_flow_rate / 60) * 4150);
  ...
  if (config.cop_calculation_method == "carnot") {
    config.heatpump_COP =
      (0.49 * (config.heatpump_flow_temperature + 4 + 273)) /
      (config.heatpump_flow_temperature +
        4 +
        273 -
        (config.T.external - 6 + 273));
  } else {
    if (config.heatpump_capacity == undefined) config.heatpump_capacity = 5.0;
    config.heatpump_COP = get_ecodan_cop(
      config.heatpump_flow_temperature,
      config.T.external,
      (config.house.total_heat_output * 0.001) / config.heatpump_capacity
    );
  }
